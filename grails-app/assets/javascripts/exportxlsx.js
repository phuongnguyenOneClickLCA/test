(function(H) {
    ...
    H.wrap(H.Chart.prototype, 'getDataRows', function(proceed, multiLevelHeaders) {
        ...
        rows = rows.filter(function(row) {
            return typeof row.x !== 'number' || (row.x >= xMin && row.x <= xMax) || (!xMax && !xMin);
        });
        return rows;
    });
    ...
    H.Chart.prototype.downloadXLSX = function() {
        ...
        var parseDateFormat
        if(this.getDataRows().length > 1){
            parseDateFormat = moment.utc(this.getDataRows()[1][0], momentFormat).creationData().format;
        } else {
            parseDateFormat = moment.utc(this.getDataRows()[0][0], momentFormat).creationData().format;
