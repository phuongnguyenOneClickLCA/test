...
%>
<div class="hide" id="lcaCheckerModal">
        ...
        <table class="table-striped table">
            ...
            <g:if test="${checksFailed && checksFailed.size()>0}">
                <g:each in="${checksFailed.keySet()}" var="key">
                    ...
                    <g:set var="rawScore" value="${checksFailed.get(key)[0]}" />
                    <g:set var="formattedScore" value="${key.getFormattedScoreForUser(user, checksFailed.get(key)[0], "0.###")}"/>
                    ...
                    <tr class="checksFailed">
                        <td>${i++}</td>
                        <td><strong>${key.checkName}: </strong>${key.getErrorText(rawScore)}</td> <%--"minimumDatapoints".equals(customCondition)--%>
            ...
            <%-- EMPTY ROW --%>
                <tr class="checksPassed"><td colspan="7"><g:message code="lcaChecker_checksPassed" /></td></tr>
            <%-- EMPTY ROW --%>
            ...
            <g:if test="${internalUser && checksNotApplicable && userService.getSuperUser(user)}">
                <tr><td colspan="7">
                    <label onclick="showHideElementsWithLabel('.hideRowTable', this)">
                        <i class="fa fa-plus"></i>
                        Check not applicable (list shown to superusers only)
                    </label>
                </td></tr>
                <g:each in="${checksNotApplicable}" var="lcaChecker">
                    <tr class="hideRowTable hide">
                        <td>${i++}</td>
